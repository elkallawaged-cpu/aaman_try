...
# 🔥 القسم التسويقي والهندسي اللي طلبته للشركات
st.markdown("""
<div class="business-section">
    ...
    <p style="font-size: 1.05rem; line-height: 1.8; color: #333;">
        الاعتماد على الذكاء الاصطناعي العام (مثل شات جي بي تي العادي) في الشركات له عيوب خطيرة، وهنا يأتي دور تقنية <b>RAG</b> لتوفير حل احترافي آمن ومخصص:
    </p>
    <ul style="font-size: 1rem; line-height: 1.8; color: #444; padding-right: 20px;">
        <li><b>🔒 أمان وسرية بياناتك 100%:</b> الملفات الحساسة وعقود شركتك لا تُرفع لتدريب الموديلات العامة، بل يتم تشفيرها والبحث داخلها محلياً وفي بيئة معزولة تماماً.</li>
        <li><b>💰 توفير مرعب في تكلفة الـ Tokens:</b> في الشات العادي، إذا رفعت كتاباً من 500 صفحة، يتم إرسال الكتاب بالكامل مع كل سؤال جديد! أما بتقنية الـ RAG، يقوم السيستم بفلترة البيانات بدقة وإرسال الفقرات الـ 4 أو 5 ذات الصلة بالسؤال فقط، مما يوفر حتى 99% من تكلفة الاستهلاك وسرعة الرد.</li>
        <li><b>🎯 القضاء على "الهلوسة" (Zero Hallucination):</b> الذكاء الاصطناعي العادي قد يخترع إجابات (يهلوس) إذا لم يعرف المعلومة. في هذا النظام، يمكنك إجباره على الالتزام الصارم بملفات الشركة فقط، ليكون الجواب دقيقاً وموثقاً بالمراجع.</li>
        <li><b>📈 القدرة على التوسع اللانهائي:</b> يمكنك ربط هذا المحرك بأرشيف شركتك بالكامل (آلاف ملفات الـ PDF، والـ Word، وقواعد البيانات لايف)، ليقوم المساعد بالبحث والربط بينها واستخراج الأرقام والتقارير في أجزاء من الثانية.</li>
    </ul>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    if st.session_state.chat_history:
        ...
        st.download_button(
            label="📥 تحميل تقرير المحادثة (TXT)",
            data=chat_text,
            file_name="rag_chat_report.txt",
            mime="text/plain"
        )
...
if st.session_state.vector_store is not None:
    ...
    with m1:
        st.markdown(f'<div class="metric-card">📄 <b>وثائق الشركة:</b> {st.session_state.meta_stats["files"]} ملفات</div>', unsafe_allow_html=True)
    with m2:
    ...
    with qp2:
        if st.button("🔍 استخرج أهم الحقائق أو الأرقام من الداتا"):
            st.session_state.quick_input = "استخرج أهم حقائق، استنتاجات أو أرقام رئيسية موجودة داخل البيانات."
    with qp3:
        if st.button("❓ اقترح عليّ أسئلة ذكية يمكنني طرحها هنا"):
